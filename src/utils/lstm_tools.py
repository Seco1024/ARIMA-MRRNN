...
class lstm_model(object):
    def __init__(self, is_doubled_layer, neurons, dropout, lr):
    ...
    def predict(self, residual_X):
        lstm_prediction = self.best_model.predict(residual_X)
    ...
    def visualize_loss_plot(self, history, today):
        ...
        plt.xlabel('# epochs')
        plt.ylabel('MSE')
        plt.savefig(os.path.join(parent_dir, f'out/LSTM_plot/{str(today)}/LSTM_{str(self.is_doubled_layer)}_{str(self.neurons)}_{str(self.dropout)}_{str(self.lr)}.png'))
    
    def evaluate_model(self, train_X, train_y, val_X, val_y, test_X, test_y, today):
        best_model = load_model(os.path.join(parent_dir, f'models/{str(today)}/LSTM_{str(self.is_doubled_layer)}_{str(self.neurons)}_{str(self.dropout)}_{str(self.lr)}.h5'))
        ...
        evaluate_df.to_csv(os.path.join(parent_dir, f'out/LSTM_error/{str(today)}/LSTM_{str(self.is_doubled_layer)}_{str(self.neurons)}_{str(self.dropout)}_{str(self.lr)}.csv'), index=False)
